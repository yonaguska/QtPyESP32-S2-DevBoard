'''
   ...
   This code supports my Adafruit QT Py ESP32-S2 development board. The
   board is designed to accomodate the integration and testing of various
   QT STEMMA connected Adafruit modules.

   I am creating modules for the
   hardware aspects of the system to simplify creation and maintenance of
   new and existing hardware, and to provide a comprehensive example for
   how to use Adafruit processors and modules. This currently runs on an
   Adafruit QT Py ESP32-S2 board with sensors hung off of the STEMMA bus.
   ...
   - DS3231     Real time clock chip with EEPROM storage
   ...
   This code was built for CircuitPython 8, and upgraded for 9.2.7.
...
version = sys.version # Working copy with sensors, RTC w/NTP, soil_probe, external battery probe, and talks to MQTT via Wifi
...
sd_card_used = True
testing_wdt = False
show_sd_contents = False # mostly the filesystem stuff ... you usually dont want to see it

# <<<<<<<<<<< IF YOU NEED TO SET THE RTC, USING NTP >>>>>>>>
set_ds3231 = False  # Set it to True to set up NTP time and then set this back to False
...
try:
    ...
    try:
        ...
        print("accessed the hardware SD card")
        try:
            vfs = storage.VfsFat(sdcard)
            print("created the vfs")
            try:
                storage.mount(vfs, "/sd")
                print("mounted vfs to the /sd directory")
                sdcard_found = True
            except Exception as ex:
                print("Error mounting vfs ... {}".format(ex))
        except Exception as ex:
            print("Error creating vfs ... {}".format(ex))
    except Exception as ex:
        print("Error with hardware access to SD card ... {}".format(ex))
except Exception as ex:
    print("Error with spi ... {}".format(ex))

# Print the card information
if sdcard_found:
    print("SD card filesystem contents are:")
    id_file = "/sd/Identifier.txt"
    try:
        with open(id_file, "r") as f:
            print("Identifier.txt contains:")
            line = f.readline()
            while line != '':
                print("\t{}".format(line))
                line = f.readline()
                if line == "show_sd_contents = True": show_sd_contents = True
        if show_sd_contents: print_directory("/sd") # print filesystem contents
    except Exception as ex:
        print("Error with reading Identifier.txt ... {}".format(ex))
...
if env_sensors_found:
    ...
    if ds3231 != None:
        status, sensor = mod_ds3231.read(ds3231)
        if status: sensors.update({sensor['type']:sensor})
